...
name: five-ws
description: >-
  Run an exhaustive Five Ws expansion for one problem or a list of problems:
  ask any combination of why, what, when, where and how, each up to five levels
  deep with five answers at every step (5x5x5x5x5 = 3,905 answers per tree),
  and write every tree to a single JSON file, plus an index and mechanical
  hygiene flags. Use when the user says "five Ws", "5 Ws", "what when where
  how", "question tree", "ask how and where about", or runs /five-ws. Supports
  --ask, --depth 1-5, --model-level 1-5 and a cheap --smoke rehearsal.
  Generates the trees only; analysis is left to the user.
...
`S="${CLAUDE_PLUGIN_ROOT}/scripts/fivews.py"` parses the invocation, decides
...
path of `fivews.py`: use it as `$S` in every later command, never a shortened
relative path such as `scripts/fivews.py`.
...
- If `yes` is false and the inputs are thin, ask the user once, in one message,
  ...
  When the user replies, write the reply verbatim to `.five-ws/context.md`
  and continue with Step 2, adding `--context-file .five-ws/context.md`.
...
```bash
python3 "$S" init <init_flags> [--context-file .five-ws/context.md] <<'FIVE_WS_INPUTS'
<input 1>
<input 2>
FIVE_WS_INPUTS
```
...
3. Otherwise dispatch every task in `tasks` in a single message, one Agent call
   ...
   - `subagent_type`: the plan's `agent` value (`five-ws:expander`)
...
directory first, then read `five-ws.json` **whole** before drawing any
