...
def hygiene(flat: list[dict], report=None) -> dict:
    ...
    for (a, b), count in shared.items():
        ...
        if score >= SIMILARITY:
            near.append(pair)
        elif score >= CROSS_BRANCH_SIMILARITY and branch_of(a) != branch_of(b):
            cross.append(pair)
    ...
    return {
        ...
        "method": "Word overlap (Jaccard) on lowercased, stemmed words minus stopwords; sibling pairs ignore "
                  ...
                  "reasons scoring between cross_branch_similarity and similarity, as leads for convergence. "
